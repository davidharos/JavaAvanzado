...
public class Filtro implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        //Se traen los datos del requetst
        String b = request.getParameter("base");
        String h = request.getParameter("altura");
       
        //Se verifican que los datos no esten nulos para evitar NullPointerExceptions
        if (b != null && h != null) {
            
             //Se parsean a double para realizar la operacion
            double bt = Double.parseDouble(b);
            double ht = Double.parseDouble(h);
            
            //Se verifica si alguno de los datos es menor a 0
            if (bt < 0 || ht < 0) {
